...
def mv_centroids(args):
    ...
    for k, i in centroids_dict.items():
        ...
        shutil.copy(
            os.path.join(i[0], i[1][0]),
            os.path.join(args.p2trackers, k.lower(), "centroids.txt"),
        )
    ...
    for k in csv_dicts.keys():
        ...
        for i, tracker in enumerate(all_trackers):
            all_subjects_dict[i] = {
                ...
                "bboxes": np.array(tracker["bboxes"]).tolist(),
                "centroids": np.array(tracker["centroids"]).tolist(),
        ...
        pd.concat([i for i in all_dfs if not i.empty], ignore_index=True).to_csv(
            os.path.join(args.p2trackers, k.lower(), "long_dfs.csv"), index=False
        )
        print(k, "done")

        # Delete det
        shutil.rmtree(os.path.join(args.p2det, k, "csv"))
