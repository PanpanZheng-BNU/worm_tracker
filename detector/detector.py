...
def detect(v_dict, p2s,roiRadius, vis, imgs, video, date):
    ...
    object_detector = cv2.createBackgroundSubtractorKNN(
        history=1500, dist2Threshold=140) # 1000 170
    ...
    if video:
        new_frame = cv2.VideoWriter(os.path.join(p2r, subj_name[0] + ".avi"),
    ...
    for v in vs:
        ...
        while curr_v.isOpened():
            df = pd.DataFrame(
                columns=["frame", "x", "y", "w", "h", "cX", "cY", "ellipse_w", "ellipse_h"])
            ...
            new_gray = cv2.rectangle(new_gray, (roi_x - roiRadius, roi_y - roiRadius),
                                    (roi_x + roiRadius, roi_y + roiRadius), (255, 255, 255), -1)
            ...
            for contour in contours:
                if 20 < cv2.contourArea(contour) < 250:
                    ...
                    df.loc[len(df)] = [n_frame, x, y, w, h,
                                       cX, cY, ellipse_w, ellipse_h]

            # cv2.circle(frame, (roi_x, roi_y), roiRadius, (255, 0, 0), 3)
            cv2.rectangle(frame, (roi_x - roiRadius, roi_y - roiRadius),
                                    (roi_x + roiRadius, roi_y + roiRadius), (255, 0, 0), 3)
            
            df.to_csv(os.path.join(p2rcsv, "{}_frame{}.csv".format(
                subj_name[0], n_frame)), index=False)
            bar.update(n_frame)
            ...
            if imgs:
                ...
                cv2.imwrite(os.path.join(p2rimg, "{}_frame{}.jpg".format(subj_name[0], n_frame)), frame,
                            [int(cv2.IMWRITE_JPEG_QUALITY), 60])
            
            (n_frame % 1000 == mod_number) and cv2.imwrite(os.path.join(p2r, "sample_img_{}.jpg".format(n_frame)), frame,
                                         [int(cv2.IMWRITE_JPEG_QUALITY), 60])
            key = cv2.waitKey(1)
            if key == ord('q'):
                break
...
if __name__ == "__main__":
    ...
    p.starmap(detect, [(i, j) for i, j in zip(
        new_dict, 1200 * np.ones(len(new_dict), dtype=int))])
