...
def find_all_videos(p2v):
    ...
    for root, dirs, files in os.walk(p2v):
        ...
        sub_id = [video.split(".")[0] for video in videos if len(video.split("_")) < 3] # get the subject id
        all_file_dict = {id: sorted([os.path.join(root, v) for v in videos if id in v]) for id in sub_id} # create a dictionary of all the videos to each subject
