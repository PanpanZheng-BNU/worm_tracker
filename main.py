...
def create_parse():
    ...
    parse.add_argument("--p2det", type=str, help="Path to store the concatenated videos", default="./detect_results")
    parse.add_argument("--pool", type=int, help="Number of processes", default=4)
    parse.add_argument("--vis", type=str2bool, help="Visualize the video", default='false')
    parse.add_argument("--img", type=str2bool, help="Whether store the detect imgs at each frame", default=False)
    parse.add_argument("--video", type=str2bool, help="whether store the videos", default=False)
    parse.add_argument("--date", type=str, help="Date of the video, to prevent name conflict and overwriting", default=datetime.strftime(datetime.now(), "%m.%d")) 

    parse.add_argument("--p2trackers", type=str, help="Path to store the simple trackers", default="./simple_trackers_result")
    return parse.parse_args()

def detect_main(args):
    dicts_list = find_all_videos(args.p2v) # obtain all paths to the videos in the specific directory
    ...
    p.starmap(detect, [(vdict, p2s, radius, vis, img, video, date) for vdict, p2s, radius, vis, img, video, date in zip(dicts_list,
